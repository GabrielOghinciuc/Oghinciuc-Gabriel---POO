...
public:
	...
	double salariu;
	double varsta;
	Pers(char* n = "Anonim ", double s = 0 , double v = 0) :salariu(s),varsta(v)
	{
		strcpy(nume, n);
	}
...
{
	double spor = 10;
	Pers p1("Marcelinho", 1,27), p2 = Pers("Trofor", 69,30);
	Pers p3("Carciumaru Florin", 233,45);
	Pers p4;
	cout << "\nDupa spor " << p3.nume << " ar avea " << p3 + spor;
	p3 += spor;
	cout << "\n" << p3.nume << " chiar are acum " << p3.salariu << " lei,are varsta de " << p3.varsta << " ani.";
	cout << "\nFamilia " << p1.nume << " are " << p1 + p2 << " lei.\n";
	cout << "\n" << p4.nume << p4 + p2 << " " << p4.varsta;
	getchar();
}
